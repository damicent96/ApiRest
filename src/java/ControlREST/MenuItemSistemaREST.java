...
import Modelo.DaoImp.MenuItemSistemaDAOIMP;
...
@Path("menuItemSistema")
public class MenuItemSistemaREST {

    private MenuItemSistemaDTO dto;
    private MenuItemSistemaDAOIMP dao;
    ...
    public String recuperarRegistro(@QueryParam("id") Integer id, @QueryParam("token") String token) {
        ...
        if (tokenDAO.verificarToken(token) == true) {
            dao = new MenuItemSistemaDAOIMP();
            MenuItemSistemaDTO dto = dao.recuperarRegistro(id);
            if (dto != null) {
    ...
    public String insertar(InputStream json, @QueryParam("token") String token) {
        ...
        tokenDAO = new TokenDAOIMP();
        Gson gson = new Gson();
        dto = gson.fromJson(Util.getJson(json), MenuItemSistemaDTO.class);
        if (tokenDAO.verificarToken(token) == true) {
            dao = new MenuItemSistemaDAOIMP();
    ...
    public String eliminar(@QueryParam("id") Integer id, @QueryParam("token") String token) {
        ...
        if (tokenDAO.verificarToken(token) == true) {
            dao = new MenuItemSistemaDAOIMP();
            Boolean resp = dao.eliminarRegistro(new MenuItemSistemaDTO(id));
            if (resp == false) {
